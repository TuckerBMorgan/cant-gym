use cant::central::{clip_gradients, get_equation, Shape, Tensor};
use cant::nn::{Layer, Linear, Model};
use cant::optimizers::AdamW;
use gym_rs::envs::classical_control::mountain_car::MountainCarObservation;
...
use super::config::{
    BATCH_SIZE, EPS_DECAY, EPS_END, EPS_START, GAMMA, GRAD_CLIP_VALUE, NUM_ACTIONS,
    OBSERVATION_SIZE, TAU,
};
...
pub fn select_action(state: Tensor, steps_done: usize, policy_net: &mut DQN) -> Tensor {
    ...
    if sample > eps_threshold {
        ...
        // Random action from 0 to NUM_ACTIONS-1 (0, 1, or 2 for MountainCar)
        let random_result = rand::thread_rng().gen_range(0..NUM_ACTIONS);

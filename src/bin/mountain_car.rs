mod dqn {
    pub use cant_gym::dqn::*;
}
mod mountain_car {
    pub use cant_gym::mountain_car::*;
}
mod tui {
    pub use cant_gym::tui::*;
}
mod utils {
    pub use cant_gym::utils::*;
}
...
use gym_rs::{core::Env, envs::classical_control::mountain_car::MountainCarEnv, utils::renderer::RenderMode};
use ratatui::prelude::*;

use cant::optimizers::AdamW;
use dqn::{create_new_model_from_existing, DQN, ReplayMemory, Transition};
use mountain_car::{
    calculate_epsilon, convert_observation_to_tensor, optimize_model, select_action, update_model,
    LR, MAX_STEPS_PER_EPISODE, MEMORY_CAPACITY, NUM_ACTIONS, OBSERVATION_SIZE, TOTAL_EPISODES,
};
use tui::{ui, DashboardState, TrainingUpdate};

#[derive(Parser, Debug)]
#[command(version, about = "MountainCar DQN Training", long_about = None)]
struct Args {
...
fn run_training(tx: Sender<TrainingUpdate>, learning_rate: f32) {
    ...
    let mut policy_net = DQN::new(OBSERVATION_SIZE, NUM_ACTIONS);
    ...
    for i_episode in 0..TOTAL_EPISODES {
        ...
        loop {
            ...
            let done = action_reward.done || action_reward.truncated || t == MAX_STEPS_PER_EPISODE;

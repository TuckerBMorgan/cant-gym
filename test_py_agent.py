...
def save_dqn_to_gguf(model: nn.Module, out_path: str, *, architecture: str = "dqn", name: str = "dqn") -> None:
...
def dump_sampled_states_and_rewards(mem: "ReplayMemory", n: int, *, prefix: str) -> None:
...
# Export GGUF once we hit 500 steps in a single episode
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        # If we manage to reach 500 steps in a single episode (CartPole-v1 max),
        # export both networks as GGUF exactly once.
        if (not saved_after_500) and (t + 1 >= 500):
            save_dqn_to_gguf(policy_net, f"policy_net_ep{i_episode + 1}_step{t + 1}.gguf", name="policy_net")
            save_dqn_to_gguf(target_net, f"target_net_ep{i_episode + 1}_step{t + 1}.gguf", name="target_net")
